...
/**
 * Service for deleting activity documents.
 */
@Service
@Slf4j
@RequiredArgsConstructor
@Transactional
public class ActivityDocumentDeleteService {
    ...
    @AuditLogAnnotation(
        action = "DELETE_ACTIVITY_DOCUMENT",
        description = "Deleting activity document",
        entityType = "ActivityDocument"
    )
    public ResponseEntity<ApiResponse<?>> deleteDocument(String obfuscatedId) {
        log.info("Deleting activity document with ID: {}", obfuscatedId);
        ...
        try {
            ...
            activityDocumentRepository.delete(document);
            ...
            log.info("Activity document deleted successfully: {}", id);

            return ResponseEntity.ok().body(
                ApiResponse.success(200, "Activity document deleted successfully", null)
            );

        } catch (Exception e) {
            log.error("Error deleting activity document", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete activity document", "DOCUMENT_DELETE_FAILED")
            );
        }
    }

    public ResponseEntity<ApiResponse<?>> deleteDocuments(List<String> obfuscatedIds) {
        log.info("Deleting {} activity documents", obfuscatedIds != null ? obfuscatedIds.size() : 0);
        ...
        try {
            ...
            ActivityDocumentDeleteService proxy = (ActivityDocumentDeleteService) AopContext.currentProxy();
            ...
            log.error("Error deleting activity documents", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete activity documents", "BULK_DELETE_FAILED")
            );
        }
    }
}
