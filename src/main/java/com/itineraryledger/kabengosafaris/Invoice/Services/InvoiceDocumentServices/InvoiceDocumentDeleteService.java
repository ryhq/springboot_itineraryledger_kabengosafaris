...
/**
 * Service for deleting invoice documents.
 */
@Service
@Slf4j
@RequiredArgsConstructor
@Transactional
public class InvoiceDocumentDeleteService {
    ...
    @AuditLogAnnotation(
        action = "DELETE_INVOICE_DOCUMENT",
        description = "Deleting invoice document",
    ...
    public ResponseEntity<ApiResponse<?>> deleteDocument(String obfuscatedId) {
        log.info("Deleting invoice document with ID: {}", obfuscatedId);
        ...
        try {
            ...
            log.info("Invoice document deleted successfully: {}", id);

            return ResponseEntity.ok().body(
                ApiResponse.success(200, "Invoice document deleted successfully", null)
            );

        } catch (Exception e) {
            log.error("Error deleting invoice document", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete invoice document", "DOCUMENT_DELETE_FAILED")
            );
        }
    }

    public ResponseEntity<ApiResponse<?>> deleteDocuments(List<String> obfuscatedIds) {
        log.info("Deleting {} invoice documents", obfuscatedIds != null ? obfuscatedIds.size() : 0);
        ...
        try {
            ...
            InvoiceDocumentDeleteService proxy = (InvoiceDocumentDeleteService) AopContext.currentProxy();
            ...
            log.error("Error deleting invoice documents", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete invoice documents", "BULK_DELETE_FAILED")
            );
        }
    }
}
