package com.itineraryledger.kabengosafaris.Quote.QuoteDay.QuoteDayFlight.Services;
...
import com.itineraryledger.kabengosafaris.Quote.QuoteDay.QuoteDayFlight.DTOs.QuoteDayFlightDTO;
...
@Transactional
public class QuoteDayFlightService {

    private final QuoteDayFlightRepository repository;
    private final QuoteDayRepository days;
    ...
    public ResponseEntity<ApiResponse<?>> getAll(String dayIdObfuscated) {
        ...
        List<QuoteDayFlight> rows = repository.findByQuoteDayIdOrderBySortOrderAscIdAsc(dayId);
    ...
    public ResponseEntity<ApiResponse<?>> create(String dayIdObfuscated, CreateQuoteDayFlightDTO dto) {
        Long dayId = idObfuscator.decodeId(dayIdObfuscated);
        QuoteDay day = dayId == null ? null : days.findById(dayId).orElse(null);
        if (day == null) {
        ...
        int nextOrder = (int) repository.countByQuoteDayId(dayId) + 1;

        QuoteDayFlight flight = QuoteDayFlight.builder()
            .quoteDay(day)
            .flightRoute(route)
            .flightFare(fare)
    ...
    public ResponseEntity<ApiResponse<?>> update(String id, UpdateQuoteDayFlightDTO dto) {
        QuoteDayFlight flight = find(id);
        if (flight == null) return notFound();
    ...
    public ResponseEntity<ApiResponse<?>> makePrimary(String id) {
        QuoteDayFlight chosen = find(id);
        if (chosen == null) return notFound();

        Long dayId = chosen.getQuoteDay().getId();
        for (QuoteDayFlight sibling : repository.findByQuoteDayIdOrderBySortOrderAscIdAsc(dayId)) {
    ...
    private QuoteDayFlight find(String id) {
    ...
    private QuoteDayFlightDTO toDTO(QuoteDayFlight flight) {
        ...
        return QuoteDayFlightDTO.builder()
            .id(idObfuscator.encodeId(flight.getId()))
            .quoteDayId(idObfuscator.encodeId(flight.getQuoteDay().getId()))
