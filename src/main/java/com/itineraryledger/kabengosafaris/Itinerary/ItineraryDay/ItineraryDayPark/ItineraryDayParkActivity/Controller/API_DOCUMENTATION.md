# Itinerary Day Park Activity API Documentation

REST API for managing activities within a park visit.

## Base URL

```
/api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities
```
...
Adds one or more activities to a park visit. The `sortOrder` is automatically determined based on existing activities. The same activity can be added multiple times (e.g., morning and evening game drives).

**URL:** `POST /api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities`

**Permission:** `PERM_UPDATE_ITINERARY_DAY_PARK_ACTIVITY`
...
| isIncludedInPrice | Boolean | No | Whether activity is included in itinerary price (default: true) |
...
| 500 | PARK_ACTIVITIES_ADD_FAILED | Failed to add park activities |

---

### Get Park Activities

Retrieves all activities for a specific park visit, ordered by `sortOrder`.

**URL:** `GET /api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities`

**Permission:** `PERM_READ_ITINERARY_DAY_PARK_ACTIVITY`
...
{
  "status": 200,
  "message": "Park activities retrieved",
  "data": [
    {
      "id": "ghi789rst",
      "itineraryDayParkId": "parkVisitId123",
...
|--------|------------|-------------|
| 400 | INVALID_ID | Invalid park visit ID format |
| 500 | FETCH_FAILED | Failed to fetch park activities |

---

### Get Single Park Activity

Retrieves a specific activity from a park visit.

**URL:** `GET /api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities/{activityId}`
...
{
  "status": 200,
  "message": "Park activity retrieved",
  "data": {
    "id": "ghi789rst",
    "itineraryDayParkId": "parkVisitId123",
    "parkId": "park123abc",
...
| 404 | PARK_ACTIVITY_NOT_FOUND | Park activity not found |
| 500 | FETCH_FAILED | Failed to fetch park activity |

---

### Update Park Activity

Updates a specific activity within a park visit.

**URL:** `PUT /api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities/{activityId}`
...
| endTime | String | No | End time (e.g., "08:30") |
| notes | String | No | Additional notes |
| isIncludedInPrice | Boolean | No | Whether activity is included in itinerary price |

**Note:** The `parkActivity` (park + activity combination) cannot be changed after creation. To change the activity, delete this record and create a new one. The `sortOrder` is handled by the reorder endpoint.

**Success Response (200):**
```json
{
  "status": 200,
  "message": "Park activity updated successfully",
  "data": {
    "id": "ghi789rst",
    "itineraryDayParkId": "parkVisitId123",
    "parkId": "park123abc",
...
| 404 | PARK_ACTIVITY_NOT_FOUND | Park activity not found |
| 500 | ITINERARY_DAY_PARK_ACTIVITY_UPDATE_FAILED | Failed to update park activity |

---

### Reorder Park Activities

Reorders activities within a park visit. Uses a two-pass approach to avoid unique constraint violations.

**URL:** `POST /api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities/reorder`
...
| 400 | NO_ACTIVITIES_TO_REORDER | Park visit has no activities to reorder |
| 404 | PARK_VISIT_NOT_FOUND | Park visit not found |
| 500 | PARK_ACTIVITIES_REORDER_FAILED | Failed to reorder activities |

---

### Delete Park Activities

Deletes one or more activities from a park visit.

**URL:** `DELETE /api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/activities`
...
|--------|------------|-------------|
| 400 | INVALID_ID | Invalid ID format |
| 500 | DELETE_FAILED | Failed to delete park activities |
...
| itineraryDayParkId | String | Obfuscated parent park visit ID |
...
| durationHours | BigDecimal | Duration in hours |
| startTime | String | Start time (e.g., "06:00") |
| endTime | String | End time (e.g., "08:30") |
| notes | String | Additional notes |
| isIncludedInPrice | Boolean | Whether activity is included in itinerary price |
...
- **Sibling - Park Tariffs:** `/api/itineraries/{itineraryId}/days/{dayId}/parks/{parkVisitId}/tariffs`
...
- The `isIncludedInPrice` flag indicates whether the activity cost is part of the itinerary package price
