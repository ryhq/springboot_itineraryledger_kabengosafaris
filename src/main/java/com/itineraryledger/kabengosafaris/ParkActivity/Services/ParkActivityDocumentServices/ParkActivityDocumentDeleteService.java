...
/**
 * Service for deleting park activity documents.
 */
@Service
@Slf4j
@RequiredArgsConstructor
@Transactional
public class ParkActivityDocumentDeleteService {
    ...
    @AuditLogAnnotation(
        action = "DELETE_PARK_ACTIVITY_DOCUMENT",
        description = "Deleting park activity document",
        entityType = "ParkActivityDocument"
    )
    public ResponseEntity<ApiResponse<?>> deleteDocument(String obfuscatedId) {
        log.info("Deleting park activity document with ID: {}", obfuscatedId);
        ...
        try {
            ...
            parkActivityDocumentRepository.delete(document);
            ...
            log.info("Park activity document deleted successfully: {}", id);

            return ResponseEntity.ok().body(
                ApiResponse.success(200, "Park activity document deleted successfully", null)
            );

        } catch (Exception e) {
            log.error("Error deleting park activity document", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete park activity document", "DOCUMENT_DELETE_FAILED")
            );
        }
    }

    public ResponseEntity<ApiResponse<?>> deleteDocuments(List<String> obfuscatedIds) {
        log.info("Deleting {} park activity documents", obfuscatedIds != null ? obfuscatedIds.size() : 0);
        ...
        try {
            ...
            ParkActivityDocumentDeleteService proxy = (ParkActivityDocumentDeleteService) AopContext.currentProxy();
            ...
            log.error("Error deleting park activity documents", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete park activity documents", "BULK_DELETE_FAILED")
            );
        }
    }
}
