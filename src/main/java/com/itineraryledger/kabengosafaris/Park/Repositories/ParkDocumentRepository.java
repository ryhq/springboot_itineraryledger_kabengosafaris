...
public interface ParkDocumentRepository extends JpaRepository<ParkDocument, Long>, JpaSpecificationExecutor<ParkDocument> {
    ...
    @Query("SELECT doc.id FROM ParkDocument doc WHERE doc.id > :currentId AND doc.park.id = :parentId ORDER BY doc.id ASC LIMIT 1")
    Optional<Long> findNextIdByParent(@Param("currentId") Long currentId, @Param("parentId") Long parentId);

    @Query("SELECT doc.id FROM ParkDocument doc WHERE doc.id < :currentId AND doc.park.id = :parentId ORDER BY doc.id DESC LIMIT 1")
    Optional<Long> findPreviousIdByParent(@Param("currentId") Long currentId, @Param("parentId") Long parentId);

    @Query("SELECT doc.id FROM ParkDocument doc WHERE doc.park.id = :parentId ORDER BY doc.id ASC LIMIT 1")
    Optional<Long> findFirstIdByParent(@Param("parentId") Long parentId);

    @Query("SELECT doc.id FROM ParkDocument doc WHERE doc.park.id = :parentId ORDER BY doc.id DESC LIMIT 1")
