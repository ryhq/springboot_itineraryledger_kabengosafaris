...
public class ItineraryDayCreateService {
     ...
     *
     * Day number is auto-determined based on existing days in the itinerary.
     * First day = 1, subsequent days increment from there.
     *
    ...
    public ResponseEntity<ApiResponse<?>> createItineraryDay(String itineraryIdObfuscated, CreateItineraryDayDTO createDTO) {
        log.info("Creating new day for itinerary: {}", itineraryIdObfuscated);
        ...
        try {
            ...
            // Count existing days for this itinerary
            long existingDaysCount = itineraryDayRepository.countByItineraryId(itineraryId);

            // Check if itinerary days are full
            if (existingDaysCount >= itinerary.getTotalDays()) {
                return ResponseEntity.badRequest().body(
                    ApiResponse.error(
                        400,
                        "Itinerary already has all " + itinerary.getTotalDays() + " days. Cannot add more days.",
                        "ITINERARY_DAYS_FULL"
                    )
                );
            }

            // Auto-determine day number (next sequential number)
            int nextDayNumber = (int) existingDaysCount + 1;

            // Create day entity (dayTag will be auto-generated by @PrePersist in entity)
            ...
            log.info("Itinerary day created successfully: Day {} (tag: {}) for itinerary {}", day.getDayNumber(), day.getDayTag(), itinerary.getCode());
