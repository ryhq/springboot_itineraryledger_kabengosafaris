...
/**
 * Service for deleting safari documents.
 */
@Service
@Slf4j
@RequiredArgsConstructor
@Transactional
public class SafariDocumentDeleteService {
    ...
    @AuditLogAnnotation(
        action = "DELETE_SAFARI_DOCUMENT",
        description = "Deleting safari document",
        entityType = "SafariDocument"
    )
    public ResponseEntity<ApiResponse<?>> deleteDocument(String obfuscatedId) {
        log.info("Deleting safari document with ID: {}", obfuscatedId);
        ...
        try {
            ...
            safariDocumentRepository.delete(document);
            ...
            log.info("Safari document deleted successfully: {}", id);

            return ResponseEntity.ok().body(
                ApiResponse.success(200, "Safari document deleted successfully", null)
            );

        } catch (Exception e) {
            log.error("Error deleting safari document", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete safari document", "DOCUMENT_DELETE_FAILED")
            );
        }
    }

    public ResponseEntity<ApiResponse<?>> deleteDocuments(List<String> obfuscatedIds) {
        log.info("Deleting {} safari documents", obfuscatedIds != null ? obfuscatedIds.size() : 0);
        ...
        try {
            ...
            SafariDocumentDeleteService proxy = (SafariDocumentDeleteService) AopContext.currentProxy();
            ...
            log.error("Error deleting safari documents", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete safari documents", "BULK_DELETE_FAILED")
            );
        }
    }
}
