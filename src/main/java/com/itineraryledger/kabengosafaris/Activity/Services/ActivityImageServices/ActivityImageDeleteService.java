...
/**
 * Service for deleting activity images.
 */
@Service
@Slf4j
@RequiredArgsConstructor
@Transactional
public class ActivityImageDeleteService {
    ...
    public ResponseEntity<ApiResponse<?>> deleteImage(String obfuscatedId) {
        ...
        try {
            ...
            log.info("Activity image deleted successfully: {}", id);

            return ResponseEntity.ok().body(
                ApiResponse.success(200, "Activity image deleted successfully", null)
            );

        } catch (Exception e) {
            log.error("Error deleting activity image", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete activity image", "IMAGE_DELETE_FAILED")
            );
        }
    }

    public ResponseEntity<ApiResponse<?>> bulkDeleteImages(List<String> obfuscatedIds) {
        log.info("Bulk deleting {} activity images", obfuscatedIds != null ? obfuscatedIds.size() : 0);
        ...
        try {
            ...
            ActivityImageDeleteService proxy = (ActivityImageDeleteService) AopContext.currentProxy();
            ...
            log.error("Error bulk deleting activity images", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(500, "Failed to delete activity images", "BULK_DELETE_FAILED")
            );
        }
    }
}
