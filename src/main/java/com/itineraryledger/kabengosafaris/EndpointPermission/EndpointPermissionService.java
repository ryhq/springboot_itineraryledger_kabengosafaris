 ...
 * - Support multiple permission models (name-based and action+entity-based)
 *
 * Permission Models:
 * 1. Direct permission name: e.g., "CREATE_USER", "READ_ROLE"
 * 2. Action + Entity: e.g., PermissionAction.CREATE + "USER"
 */
@Service
...
public class EndpointPermissionService {
    ...
    public boolean canUserAccessEndpoint(User user, String httpMethod, String endpoint) {
        ...
        // Check permission-based access (Model 2: action + entity)
        if (permission.getAction() != null && permission.getEntity() != null) {
            boolean hasPermission = user.hasPermission(permission.getAction(), permission.getEntity());

            if (!hasPermission) {
                log.warn("Access denied: user {} cannot '{}' on entity '{}' for {} {}",
                    user.getUsername(), permission.getAction(), permission.getEntity(),
                    httpMethod, endpoint);
                return false;
            }

            log.debug("User {} can '{}' on entity '{}' for {} {}",
                user.getUsername(), permission.getAction(), permission.getEntity(),
                httpMethod, endpoint);
            return true;
        }
    ...
    /**
     * Register a new endpoint permission mapping with action+entity
     *
     * @param httpMethod HTTP method
     * @param endpoint endpoint path
     * @param action permission action enum (or null)
     * @param entity entity name (or null)
     * @return created EndpointPermission
     */
    @CacheEvict(cacheNames = "endpointPermissions", allEntries = true)
    @Transactional
    public EndpointPermission registerEndpointByAction(String httpMethod, String endpoint,
                                                       PermissionAction action, String entity) {
        return registerEndpoint(httpMethod, endpoint, null, action, entity, false);
    }
    ...
    private EndpointPermission registerEndpoint(String httpMethod, String endpoint,
        ...
        EndpointPermission mapping = EndpointPermission.builder()
            ...
            .action(action)
            .entity(entity)
            .requiresAuth(requiredPermissionName != null || action != null || entity != null)
     ...
     */
    public List<EndpointPermission> getEndpointsByEntity(String entity) {
