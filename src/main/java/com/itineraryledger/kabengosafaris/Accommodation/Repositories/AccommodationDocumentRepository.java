...
public interface AccommodationDocumentRepository extends JpaRepository<AccommodationDocument, Long>, JpaSpecificationExecutor<AccommodationDocument> {
    ...
    @Query("SELECT doc.id FROM AccommodationDocument doc WHERE doc.id > :currentId ORDER BY doc.id ASC LIMIT 1")
    Optional<Long> findNextId(@Param("currentId") Long currentId);

    @Query("SELECT doc.id FROM AccommodationDocument doc WHERE doc.id < :currentId ORDER BY doc.id DESC LIMIT 1")
    Optional<Long> findPreviousId(@Param("currentId") Long currentId);

    @Query("SELECT doc.id FROM AccommodationDocument doc ORDER BY doc.id ASC LIMIT 1")
