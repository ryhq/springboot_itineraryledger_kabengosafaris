...
/**
 * Service for deleting accommodation images.
 *
 * Provides:
...
@Slf4j
@Transactional
public class AccommodationImageDeleteService {
    ...
    @Autowired
    public AccommodationImageDeleteService(
    ...
    /**
     * Bulk delete accommodation images by their IDs
    ...
    public ResponseEntity<ApiResponse<?>> bulkDeleteImages(List<String> obfuscatedIds) {
        log.info("Deleting {} accommodation images", obfuscatedIds != null ? obfuscatedIds.size() : 0);
        ...
        try {
            ...
            for (Long imageId : imageIds) {
                ...
                if (image != null) {
                    ...
                    AccommodationImageDeleteService proxy = (AccommodationImageDeleteService) AopContext.currentProxy();
            ...
            if (deletedCount > 0) {
                return ResponseEntity.ok().body(
                    ApiResponse.success(
                        200,
                        deletedCount + " accommodation image(s) deleted successfully",
                        null
                    )
                );
            } else {
                return ResponseEntity.badRequest().body(
                    ApiResponse.error(
                        400,
                        "No accommodation images were deleted. " + notFoundIds.size() + " image(s) not found",
                        "NO_IMAGES_DELETED"
                    )
                );
            }
            ...
            log.error("Error deleting accommodation images", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.error(
                    500,
                    "Failed to delete accommodation images",
                    "ACCOMMODATION_IMAGE_DELETE_FAILED"
                )
            );
        }
    }
    ...
    /**
     * Delete a single accommodation image by ID (with audit logging)
    ...
    @AuditLogAnnotation(
        ...
        description = "Deleting accommodation image",
        entityType = "AccommodationImage",
        entityIdParamName = "id"
    )
    public void deleteSingleImage(Long id, String fileName) {
        log.info("Deleting accommodation image with ID: {}", id);

        // Delete from database
        accommodationImageRepository.deleteById(id);
